...
// Achievement items follow the reference markup's per-item styling:
// - a plain string that matches "Label: detail" gets the label bolded
// - { bold: '...' } renders the entire bullet bold
// - { label, text } bolds just the label fragment before the rest
// - a plain string with no colon renders unstyled
function renderAchievement(item) {
  if (typeof item === 'object') {
    if (item.bold) return item.bold
    if (item.label) {
      return (
        <>
  ...
  const match = /^([^:]{2,60}):\s*(.*)$/s.exec(item)
...
function isFullyBold(item) {
  return typeof item === 'object' && !!item.bold
}
  ...
  return (
    <div className="bg-white border border-black/[0.16] rounded-[12px] overflow-hidden flex flex-col md:flex-row w-full">
      ...
      <div className="flex-1 p-5 md:p-6">
        {achievementsHeading && (
          <p className="font-bold text-[15px] md:text-[18px] text-[#191919] mb-3">{achievementsHeading}</p>
        )}
        <ul className="list-disc pl-5 md:pl-6 space-y-2 md:space-y-3 text-[15px] md:text-[18px] text-[#191919]">
          {achievements.map((item, index) => (
            <li key={index} className={isFullyBold(item) ? 'font-semibold' : undefined}>
